  ...
  group('SubwordTokenizer', () {
    group('initialization', () {
      test('should initialize with special tokens in vocabulary', () {
        expect(
          tokenizer.vocabulary.containsToken(SubwordTokenizer.unkToken),
          isTrue,
        );
        expect(
          tokenizer.vocabulary.containsToken(SubwordTokenizer.padToken),
          isTrue,
        );
        expect(
          tokenizer.vocabulary.containsToken(SubwordTokenizer.startToken),
          isTrue,
        );
        expect(
          tokenizer.vocabulary.containsToken(SubwordTokenizer.endToken),
          isTrue,
        );
      });
      ...
      test('should initialize with common prefixes and suffixes', () {
        ...
        expect(tokenizer.vocabulary.containsToken('un'), isTrue);
        expect(tokenizer.vocabulary.containsToken('re'), isTrue);

        // Test some common suffixes
        expect(tokenizer.vocabulary.containsToken('ing'), isTrue);
        expect(tokenizer.vocabulary.containsToken('ed'), isTrue);
      });
    });
    ...
    group('_splitIntoSubwords', () {
      ...
      test('should split words with common prefixes', () {
        ...
        // Should split into 'un', 'happ', and 'y' because 'y' is a suffix
        expect(tokens.contains('un'), isTrue);
        expect(tokens.length, 3);
        expect(tokens, ['un', 'happ', 'y']);
      });
    ...
    group('encode', () {
      ...
      test('should add tokens to vocabulary', () {
        ...
        expect(
          tokenizer.vocabulary.getVocabularyMap().keys.any(
            (k) => k == 'hello' || k.contains('hell'),
          ),
          isTrue,
        );
        expect(
          tokenizer.vocabulary.getVocabularyMap().keys.any(
            (k) => k == 'world' || k.contains('worl'),
          ),
          isTrue,
        );
      });
